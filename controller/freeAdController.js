...
exports.createAd = async (req, res) => {
    try {
        const { title, category, description, phone, email, photo1, duration } = req.body;
        ...
        const requiredFields = ["title", "category", "description", "phone", "photo1"];
        ...
        if (duration) {
            if (typeof duration !== 'number') {
                return res.status(400).json({ message: "Invalid duration, expecting number of days" });
            }
        }
